...
| 무사고 문구의 기준 | `/why-us`의 `zero safety incidents`를 `zero OSHA-recordable incidents`로 좁힐지 미정. 보유 기록이 OSHA 300 로그 기준이면 후자가 더 정확하고 방어력도 높음. 경미한 응급처치 건 포함 여부가 모호한 상태 (PR #8) |
| 모바일 실기기 확인 | PR #6·#8의 렌더 검증을 headless Chrome으로만 수행. 실제 모바일 뷰포트가 적용되지 않아 좁은 폭 레이아웃은 미검증 (건드리지 않은 페이지도 동일하게 나와 회귀는 아님) |
...
| #8 | 광고 문구 실증 감사 — 근거 없는 수치 4건 제거(`50+ 프로젝트`·`100% 고객만족`·`A+ 등급`·`decades`), ADA `"meets all standards"` 보증을 시공 기준(CBC 11B / ADA Standards) 명시로 교체, CSLB 기관명 오기 정정 + Class B 분류 명시. 히어로 오버레이 대비 실측 4.06:1 → 5.84:1로 WCAG AA 미달 해소 |
...
> PR #4 행은 아직 main에 없습니다. 해당 행을 추가하는 PR #5가 PR #7과 같은 표의 같은 위치를
> 수정해 **현재 충돌(CONFLICTING) 상태**입니다. PR #5에서 main을 머지한 뒤 `#4` 행을 `#6` 위에
> 놓으면 해소됩니다.

> PR #6은 포트폴리오 데이터를 바꾸지 않은 인프라 작업이지만, 사이트 반영 이력을 한곳에서
> 추적하기 위해 같은 표에 기록합니다. PR #8은 성격이 다릅니다 — 게시된 주장을 근거와 대조하는
> 작업이라 이 문서의 목적에 그대로 부합합니다.
